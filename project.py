...
def fbconnect():
    ...
    token = data['access_token'] #'access_token=' + data['access_token']
    print("FB Token is %s" %token)
...
def gconnect():
    ...
    url = ('https://www.googleapis.com/oauth2/v1/tokeninfo?access_token=%s'
           % access_token)
    h = httplib2.Http()
    #result = json.loads(h.request(url, 'GET')[1]) # Does not work
...
def gdisconnect():
    ...
    if result['status'] == '200':
        #print("Login session keys", login_session.keys())
...
def editRestaurant(restaurant_id):
  ...
  if editedRestaurant.user_id != login_session['user_id']:
      return "<script>function myFunction(){alert('You are not authorized to edit details of this restaurant.');}</script><body onload='myFunction()'>"
  if request.method == 'POST':
...
def showMenu(restaurant_id):
    ...
    #print("Creator is", creator.id, "and login_session user_id is", login_session['user_id'],
          #"and login session is", login_session['username'])
    if 'username' not in login_session or creator.id != login_session['user_id']:
        print("rendering public menu")
        return render_template('publicmenu.html', items = items, restaurant = restaurant, creator = creator)
    else:
        print("rendering private menu")
...
def deleteMenuItem(restaurant_id,menu_id):
    ...
    if  request.method == 'POST':
            ...
            return redirect(url_for('showMenu', restaurant_id = restaurant_id))
        #else#flash('Menu Item Deletion Cancelled')
    else:
